...
facts.json 是第一阶段的数字与催化边界。正文和第二阶段 payload 只能使用这里登记且裁定可用的事实，或 `derive_facts.py` 从原始取数字段复算出的派生值。
...
| `facts[]` 为空 | 登记会进入正文 / 仪表盘的展示数字和最终入选催化 |
...
第二阶段只能通过 `hydrate_payload_from_facts.py` 复用这张表，不得在 payload 重新分组。
...
- 第二阶段 payload：展示项用 `fact:"id"` 与 `value_num` 绑定；`numeric_facts` 与 facts.json 同源。
- 10 股 `stock_checks` 与 `divergence_groups` 通过 `hydrate_payload_from_facts.py` 自动映射到 payload，不在第二阶段重复填。
