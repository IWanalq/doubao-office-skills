# 第二阶段 payload 合同（默认读取）

仅当用户要求“生成HTML”后读取本文件。字段全集与长解释见 `payload_fields.md`；示例 payload 是测试资产，默认不要读 `assets/example_payload.json`。

## 1. 第二阶段原则

- HTML 内容以第一阶段对话框正文为唯一母版；不重新取数、不重算、不改核心判断、不另写一版分析。
- 第二阶段只允许把第一阶段正文**原句搬运**并字段化进 payload；不允许压缩、改写、同义改写或重新概括。payload 某段内容在正文中找不到原文片段时，回第一阶段补正文并重新 lint，不在 HTML 阶段现场发挥。
- 10 股行情、近7日复算、`role/select_reason`、四组分组来自第一阶段 facts；第二阶段不得手工重填。
- payload 生成后必须先水合，再校验，再渲染。
...
`hydrate_payload_from_facts.py` 会覆盖 `stocks` / `divergence_groups[].stocks` 并写入 `_facts_hydration.stocks_sha256`；同时会从同目录 `<板块>_分析草稿.md` 自动抽取模块3四组解释写入 `divergence_groups[].feature`。缺指纹或水合后被手工改动，渲染门会报错；若草稿路径不标准，运行水合时用 `--chat work/<板块>_分析草稿.md` 显式传入。

`check_payload_against_chat.py` 会检查 payload 文本是否是第一阶段正文对应模块中的原文片段；失败时不要改 HTML 文案凑过，回到第一阶段正文补足内容。

## 3. chat → payload 映射

第二阶段按下表映射；所有文本字段必须原句复制或截取第一阶段正文中的连续原文片段，不做压缩、不做改写、不新增判断。
...
| 模块6来源 | `sources[]` | 放正文实际使用的公开来源与数据终端；`name` 填来源名，`title` 填模块6原句中的标题 / 用途，`date` 填日期；有 URL 的展示链接，无 URL 的展示为数据终端 |
...
`value` 逐字搬运模块1表格的“数值”列，不能压缩成单个裸数字；它应是紧凑证据短句：价格写点位与当日 / 近7日涨跌对比，成交写当日成交额与近7个交易日日均成交额 / 放缩量，代表股写 10 股覆盖面与领涨 / 拖累结构，估值写 10 股 PE(TTM) 中位数 / 区间 / 分布。
估值位置只按 10 只代表股 PE(TTM) 分析，不写板块 PE；如果 value 含 PE/PB/PS，必须写 TTM / 静态 / 动态 / LYR / 滚动等口径与时点。
...
来源只许一级 / 二级。个人 / 自媒体 / 无法确认机构作者不得入选。聚合 / 分发承载页若机构署名，只能标二级；标一级会报错。
