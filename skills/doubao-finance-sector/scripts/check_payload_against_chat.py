...
def _check_text(label: str, value: str, context: str, errors: list[str]) -> None:
    ...
    if FIXED_RISK_NOTICE in str(value):
        errors.append(f"{label} 不得搬运第一阶段固定风险提醒；HTML 使用渲染脚本内置免责声明。")
        return
    if not _supported(str(value), context):
        errors.append(f"{label} 不是第一阶段正文对应模块中的连续原文片段；请从 chat 原句搬运，不要压缩、改写或在 HTML 阶段新增判断。")


def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="检查 payload 文本是否从第一阶段 chat 正文映射。")
    ap.add_argument("chat_md", help="第一阶段分析草稿 .md")
    ap.add_argument("payload_json", help="第二阶段 payload.json")
    ...
    if not opening.strip():
        errors.append("chat 开篇核心结论为空，无法映射 HTML hero。")
    ...
    if not title_text:
        errors.append("chat 开篇缺少 `标题：...`，无法稳定映射 payload.headline 到 HTML Hero 标题。")
    if payload.get("headline"):
        ...
        if re.search(r"综合热度|信息热度|行情热度|[1-5]\s*/\s*5", str(payload.get("headline"))):
            errors.append("headline 不应包含综合热度 / 信息热度 / 行情热度分数；打分已由 HTML 仪表盘和双轨图表达。")
    ...
    if not errors:
        infos.append("payload 文本映射检查通过：HTML 内容均可在第一阶段 chat 中找到连续原文片段。")
