...
# 两个搜索工具对应的来源 lane 取值（与 render_dashboard.py 一致）
...
def _date_key(s):
    ...
    与 render_dashboard._any_date_key 行为一致（用于 as_of ≤ timestamp 的时点比较）。
    """
    if not s:
        return (0, 0, 0)
    s = str(s)
...
def recompute(p):
    ...
    10 股逐股复算与量价分组属于第一阶段 facts_json_checks()，第二阶段不在
...
def hard_checks(p):
    ...
    第二阶段只做 numeric_facts / key_chips / dimensions 等展示数字的复算与登记一致性。
