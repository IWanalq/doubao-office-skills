#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""把第一阶段 facts.json 中已验证的 10 股行情与分组映射进第二阶段 payload。

第二阶段不应重新手填 stocks / divergence_groups 的行情字段；这些内容已在
...
def extract_divergence_features(chat_text: str, groups: dict) -> tuple[dict[str, str], str]:
    mod = _section(chat_text, "模块3 · 谁在动、谁没动", "模块4 · 接下来盯什么")
    if not mod:
        return {}, ""
...
def extract_sources_from_chat(chat_text: str) -> list[dict[str, str]]:
    mod = _section(chat_text, "模块6 · 信息来源", "")
    if not mod:
        m = re.search(r"^##\s*模块\s*6[ ·・.．、-]*信息来源\s*$", chat_text, re.M)
        if m:
            mod = chat_text[m.end():]
    if not mod:
        return []
...
def main(argv=None) -> int:
    ...
    ap.add_argument("payload", help="第二阶段 payload 草稿 JSON")
    ...
    print("下一步：先运行 check_payload_against_chat.py 确认 HTML 文本从 chat 原句搬运，再运行 check_market_facts.py 与 render_dashboard.py --check-only。")
    return 0
