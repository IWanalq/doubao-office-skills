...
META_LEAK_RE = re.compile(
    r"按时间倒序|最新在最上|每条标题前显示|量化信息看热力图|无对应个股.*填[\"“]?无|"
...
FIXED_CLOSING = '下一步是否为您生成完整的板块热度仪表盘？如果需要，请回复"生成HTML"。'
...
def _section_lines(lines, heading_key):
    ...
    for i, raw in enumerate(lines):
        s = raw.strip()
        if MODULE_HEADING_RE.match(s) and heading_key in s:
            start = i + 1
            break
    if start is None:
        return []
...
def _module2_event_dates(lines, facts_obj):
    ...
    for i, raw in enumerate(lines, start=1):
        s = raw.strip()
        if s.startswith("## "):
            if "模块2" in s and ("为什么涨" in s or "为什么跌" in s or "为什么涨 / 跌" in s):
                in_module2 = True
                continue
            if in_module2:
                break
...
def check_outline_contract(lines, errors):
    ...
    if not title_line:
        errors.append("开篇必须单独写一行 `标题：<一句话核心判断>`，用于映射 HTML Hero 标题；不要从打分句里抽标题。")
    elif re.search(r"综合热度|信息热度|行情热度|[1-5]\s*/\s*5", title_line):
        errors.append("开篇 `标题：` 不应包含综合热度 / 信息热度 / 行情热度分数；打分已映射到 HTML 仪表盘和双轨图。")
    if FIXED_CLOSING not in nonempty[-1]:
        errors.append('正文最后必须逐字输出：下一步是否为您生成完整的板块热度仪表盘？如果需要，请回复"生成HTML"。')
    ...
    for h in REQUIRED_HEADINGS:
        try:
            ...
            missing.append(h)
    if missing:
        errors.append(f"正文缺少固定模块标题：{'、'.join(missing)}。")
    elif positions != sorted(positions):
        errors.append("正文模块标题顺序必须固定：直接回答 / 模块1 / 模块2 / 模块3 / 模块4 / 模块5 / 模块6。")
